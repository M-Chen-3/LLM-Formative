...
with open("MBPP Bug Metric.txt", "r", encoding="utf-8") as accuracy:
    ...
    while i < len(acc_list):
        ...
        if " Please only" in text:
            # Isolate original prompt from engineered prompt
...
index = 0
for line in data:
    # Ensure that prompt exactly matches ground truth
    if line["prompt"].strip(" ").replace("python ", "") == generated_text[g_index][0].replace("python ", ""):
        ...
        # Run generated code. Passes test only if no error
        try:
            exec(generated_code)
            valid += 1
        ...
        g_index += 1
    # Debugging code for when the prompt doesn't match ground truth
    # else:
        # print(line["prompt"].strip(" ").replace("python ", "") + "sansy")
        # print(generated_text[g_index][0].replace("python ", "") + "sansy")
    index += 1
